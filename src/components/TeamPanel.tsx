...
import { Player, TeamSide, Dictionary } from '../types';
...
export const TeamPanel: React.FC<TeamPanelProps> = ({
  ...
  const title = side === 'home' ? dict.homeTeam : dict.awayTeam;
    ...
    <div className="flex flex-col bg-gray-800 p-2 md:p-4 w-full">
      <div className={`flex flex-col mb-4 w-full ${alignClass}`}>
        ...
        <div className="flex items-center gap-3 bg-gray-700 p-2 rounded-lg w-full">
          <label className="text-sm text-gray-300 font-medium whitespace-nowrap">{dict.kitColor}</label>
        ...
        {teamPlayers.map((player) => (
          <div key={player.id} className="flex gap-2 bg-gray-700/50 p-2 rounded hover:bg-gray-700 transition-colors items-center">
            <div className="w-10 md:w-12 shrink-0">
              <label className="text-[10px] text-gray-400 block mb-0.5">{dict.playerNo}</label>
              <input
                type="text"
                value={player.number}
            ...
            <div className="flex-1 min-w-0">
              <label className="text-[10px] text-gray-400 block mb-0.5">{dict.playerName}</label>
              <input
                type="text"
                value={player.name}
